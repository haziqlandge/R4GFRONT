...
| ~~6~~ | ~~Toggle chunking strategy, re-run the same query~~ **CANNOT BE SHOT. F13 was never built** (`Phases.md` Phase 8). Replace with: cut to the documentation page's chunking section, SEVEN strategies side by side with the C7 answer-key leak drawn in the refusal colour | Requirement 2 is still covered, and the leak finding is a stronger 15 seconds than a toggle would have been |
