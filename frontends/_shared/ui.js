 ...
 */
/**
 * The external panel's body.
 *
 * THIS FUNCTION NEVER HIDES THE PANEL. Visibility belongs to the MODE and is
 * owned by setAsideVisible(): in accurate mode the frame stays on screen for the
 * whole session and only its contents change. It used to hide itself between
 * questions, which meant the layout below it jumped up and then back down on
 * every single prompt - the answer and the timing panels visibly re-flowed while
 * a reader was looking at them. Clearing the text costs nothing and the frame
 * holds its place.
 *
 * Four states, in the order they are checked:
 *
 *   rateLimited  the one refusal the page speaks about (see below)
 *   text         an answer arrived
 *   pending      a question is in flight
 *   otherwise    idle: accurate mode is on but nothing has been asked yet
 */
export function renderAside(el, text, model, rateLimited = false, pending = false) {
  if (!el) return;
  ...
  //
  // Every other reason there is no answer - no key, a dead upstream, an open
  // breaker - stays quiet, because none of them is the visitor's doing and a
  ...
  if (rateLimited) {
    body.innerHTML = `
      <div class="sh-aside" data-state="limited">
        ${head}
        <p class="sh-aside-text">Too many frequent requests! You are being Rate Limited ⚠</p>
      ...
      <div class="sh-aside">
        ...
        <p class="sh-aside-foot">
          unverified information${
            model ? ` · generated by external source: ${esc(model)}` : ""}
        </p>
      </div>`;
    return;
  }
...
/**
 * Whether the external frame is on screen at all.
 *
 * Purely a function of MODE. Fast calls nothing out, so the frame would be a
 * permanent empty box; accurate always will, so it stays put and its contents
 * change underneath. Separating this from renderAside is what stops the panel
 * appearing and disappearing on every prompt.
 */
export function setAsideVisible(el, on) {
  if (!el) return;
  el.hidden = !on;
  if (!on) {
    const body = el.querySelector(".win-body");
